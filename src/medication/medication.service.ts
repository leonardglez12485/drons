...
export class MedicationService {
  ...
  async create(dto: CreateMedicationDto, file: string) {
    ...
    if (await this.itsMedication(dto.names))
      throw new BadRequestException('This Medication exist !!!');
    try {
  ...
  async findOne(term: string): Promise<Medication> {
    ...
    if (!medication)
      throw new NotFoundException(`The Medication ${term} doesn't exist !!!`);
  ...
  getStaticProductImage(imageName: string) {
    ...
    if (!existsSync(path))
      throw new BadRequestException('No image for this midecation !!!!');
    return path;
  }
  ...
  private handleException(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(
        `Medication exists in DB ${JSON.stringify(error.keyValue)}`,
      );
    }
    console.log({ error });
    throw new InternalServerErrorException(
      `Can't create Medication - Check server logs `,
    );
  }
}
